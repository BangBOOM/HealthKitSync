...
松手时停止音频采集并调用 `endAudio()`，保留音频会话直到识别完成，再取消任务并释放会话。最终结果等待上限为 10 秒；服务报错但已有文字时保留文字，不回滚整段输入。无文字失败时显示苹果返回的具体错误和错误码。

合成中文音频在 iPhone 的设备端、自动识别两种模式下均成功返回“今天做了20个俯卧撑平板支撑1分钟”，说明该手机的中文识别服务可用。Debug 参数 `--dictation-transcription-probe` 从 Documents/dictation-fixture.caf 读取合成音频，不采集麦克风；结果输出到 Documents/dictation-transcription-probe.json。

为排查真实录音路径，仅覆盖保存最近一次的技术诊断：采集帧数、采样率、峰值音量、时长、文字长度和错误码，不保存声音或文字内容。用户退出 Mac 的 iPhone 镜像、直接在手机操作后确认识别可用；Apple 的 iPhone 镜像不支持麦克风输入。

### 输入栏交互打磨

重置聊天移到记录页右上角，保留确认与草稿保护。左侧麦克风切换到整条输入栏「按住说话」，松手完成、上滑取消；识别文字不自动发送、不自动弹键盘。文字模式下向下滑动输入栏收起键盘，移除单独的收键盘按钮。

按压由原生 UIControl 跟踪，按下直接触发 rigid 触感并改变底色与按压状态。已授权时跳过重复授权请求，录音启动延后 50 毫秒，避免同步音频初始化阻塞按下反馈。音量反馈遵循减少动态效果设置。

最新 Debug 真机构建通过并安装。`--dictation-probe` 现验证已授权快速启动路径，连续三次立即松手均未启动录音且保留原草稿；它不再验证授权回调。最新按压触感延迟及完整滑动交互仍需用户真机体验确认。
