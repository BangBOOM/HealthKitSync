...
App 默认仍打开「健康同步」。新增的「记录」和「设置」不要求健康授权。

- 「记录」显示选定日期的俯卧撑次数、平板秒数和明细。点汇总卡片新增，点可编辑的明细修改；日期可以改为昨天或其他日期。
- 下方输入一句话即可展开助手。使用系统键盘，输入法语音也可用，没有独立录音功能。
- 助手卡片来自真实工具返回，区分已保存、待同步和失败；卡片按钮返回对应日期查看和修改。
- 手动新增先原子写入本机文件，再同步。网络恢复且 App 位于前台时重试；关闭助手不删除正式记录。
- 尚未提交的本地新增可以离线修改。开始提交后冻结请求，先核对结果；已同步记录修改要求成功联网读取。
...
模型配置可以留空，手动记录和健康上传继续可用。无网或模型失败时保留文本，不假装已理解或已保存。
...
flowchart LR
  ...
  Manual[手动记录页面] --> Ops[RecordStore / RecordToolService]
  Text[文本助手面板] --> Pi[内置 Pi / WKWebView]
  Pi --> Bridge[Swift 消息桥]
  Bridge --> Ops
...
- `HealthKitSync/Records/`：输入校验、Asia/Shanghai 日期、分类复用、操作日志、远端接口与按代码计算的汇总。手动入口和三个 Agent 工具共用这一层。
